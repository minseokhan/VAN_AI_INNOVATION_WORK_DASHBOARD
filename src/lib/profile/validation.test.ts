...
const empty = { level: "", skills: "", interests: "", bio: "" };

describe("validateProfile", () => {
  it("빈 입력은 모두 null로 저장된다", () => {
    expect(validateProfile(empty)).toEqual({ ok: true, value: { level: null, skills: null, interests: null, bio: null } });
  });

  it("공백만 있는 값도 null", () => {
    const r = validateProfile({ ...empty, skills: "   \n  " });
    expect(r).toEqual({ ok: true, value: { level: null, skills: null, interests: null, bio: null } });
  });

  it("정상 입력은 trim 후 그대로", () => {
    const r = validateProfile({ level: "INTERMEDIATE", skills: " Next.js ", interests: "AI 에이전트", bio: "블로그: https://a.b" });
    expect(r).toEqual({
      ok: true,
      value: { level: "INTERMEDIATE", skills: "Next.js", interests: "AI 에이전트", bio: "블로그: https://a.b" },
    });
  });
  ...
  it("길이 제한을 넘으면 필드별 에러", () => {
    ...
    expect(r.ok).toBe(false);
    if (!r.ok) expect(Object.keys(r.errors).sort()).toEqual(["bio", "interests", "skills"]);
  });
